...
export default function SongsSection({ value, setValue, genres, data, isSong = false}) {
  return (
    <Box sx={{ width: "100%", typography: "body1" }}>
      <TabContext value={value}>
        ...
        <TabPanel value="all" sx={{ px: 0 }}>
          <CardSwiper albums={data} isSong={isSong} />
        </TabPanel>

        {genres?.map((genre) => (
          <TabPanel key={genre.key} value={genre.key} sx={{ px: 0 }}>
            <CardSwiper
              albums={data?.filter((item) => item.genre.key === genre.key)}
              isSong={isSong}
            />
          </TabPanel>
        ))}
